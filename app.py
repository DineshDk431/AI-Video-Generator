...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        if mode == "Local (Offline)":
            ...
            model_options = {
                "DAMO T2V (1.7B) - Original": {
                    "id": "damo-vilab/text-to-video-ms-1.7b",
                    "vram": "~8GB",
                    "desc": "Original Alibaba DAMO model. Stable, well-tested. Good for abstract/artistic videos.",
                    "icon": "📊"
                },
                "Wan 2.1 (1.3B) - Balanced": {
                    "id": "Wan-AI/Wan2.1-T2V-1.3B-Diffusers",
                    "vram": "~8GB",
                    "desc": "Newer Alibaba Wan model. Better quality, more realistic. Ideal for most users.",
                    "icon": "⚡"
                }
            }
            ...
            selected_model = st.selectbox(
                ...
                index=0,
                help="Choose based on your GPU VRAM"
            )
...
def generate_video(prompt: str, settings: dict, status_placeholder):
    ...
    if settings.get("mode") == "Cloud (Faster)":
        status_placeholder.markdown('<span class="status-badge status-generating">☁️ Connecting to Cloud Queue (Firebase)...</span>', unsafe_allow_html=True)
        
        try:
            from utils.firebase_utils import submit_job_to_cloud, get_job_status
            result = submit_job_to_cloud(prompt, settings)
            
            if result["success"]:
                job_id = result['job_id']
                st.session_state.cloud_job_id = job_id
                st.session_state.cloud_status = {"status": "pending"}
                
                # Save to persistent storage
                from utils.storage import save_cloud_job_id
                save_cloud_job_id(job_id, prompt)
                
                status_placeholder.markdown(f'<span class="status-badge status-generating">☁️ Job Queued: {job_id[:8]}... Waiting for cloud worker...</span>', unsafe_allow_html=True)
                ...
                for attempt in range(max_attempts):
                    ...
                    status_placeholder.markdown(
                        f'<span class="status-badge status-generating">☁️ Cloud generating... ({elapsed}s elapsed)</span>', 
                        unsafe_allow_html=True
                    )
                    ...
                    if job_data:
                        ...
                        st.session_state.cloud_status = job_data
                        
                        if status == "completed":
                            video_url = job_data.get("video_url")
                            if video_url and os.path.exists(video_url):
                                # Video is ready and accessible locally
                                status_placeholder.markdown('<span class="status-badge status-ready">✅ Cloud Video Complete!</span>', unsafe_allow_html=True)
                                
                                # Copy to outputs folder for consistency
                                from utils.video import ensure_output_dir, generate_filename
                                ...
                                save_to_history(prompt, "cloud", output_path, settings)
                                
                                return output_path
                            elif video_url:
                                # Video URL exists but may be remote - try to display directly
                                status_placeholder.markdown('<span class="status-badge status-ready">✅ Cloud Video Complete!</span>', unsafe_allow_html=True)
                                st.session_state.cloud_video_url = video_url
                                
                                # Save to history with remote URL
                                from utils.storage import save_to_history
                                save_to_history(prompt, "cloud", video_url, settings)
                                
                                return video_url
                            else:
                                status_placeholder.markdown('<span class="status-badge" style="background: rgba(245, 158, 11, 0.2); color: #f59e0b;">⚠️ Video URL missing from response</span>', unsafe_allow_html=True)
                                return None
                                
                        elif status == "error":
                            error_msg = job_data.get("error", "Unknown error occurred")
                            status_placeholder.markdown(f'<span class="status-badge" style="background: rgba(239, 68, 68, 0.2); color: #ef4444;">❌ Cloud Error: {error_msg}</span>', unsafe_allow_html=True)
                            st.error(f"Cloud generation failed: {error_msg}")
                            return None
                        
                        elif status == "processing":
                            status_placeholder.markdown(
                                f'<span class="status-badge status-generating">🔄 Processing on cloud... ({elapsed}s)</span>', 
                                unsafe_allow_html=True
                            )
                
                # Timeout reached
                status_placeholder.markdown('<span class="status-badge" style="background: rgba(245, 158, 11, 0.2); color: #f59e0b;">⏱️ Cloud generation taking longer than expected. Check sidebar for status.</span>', unsafe_allow_html=True)
                st.info(f"Job ID: {job_id}\nThe video is still generating. Use 'Check Now' in the sidebar to monitor progress.")
                return None
                
            else:
                msg = result['message']
                print(f"CLOUD ERROR: {msg}")
                st.error(f"❌ Cloud Generation Failed: {msg}")
                st.warning("👉 If this persists, please switch to 'Local Mode' in the sidebar.")
                return None
        except ImportError:
            st.error("⚠️ Missing 'firebase-admin'. Please run: pip install firebase-admin")
            return None
    
    # ----------------LOCAL MODE----------------
    try:
        # Get selected model name for display
        model_id = settings.get("model_id", "Wan-AI/Wan2.1-T2V-1.3B-Diffusers")
        model_name = model_id.split("/")[-1] if model_id else "Wan2.1-T2V-1.3B"
